...
pub fn generator(input: &str) -> (i64, i64, Vec<i64>) {
    fn inner(input: Vec<i64>) -> (i64, i64, Vec<i64>) {
        (*input.iter().min().unwrap(),*input.iter().max().unwrap(), input)
    }
    input
        .split(",")
...
pub fn solve_part1((min, max, input): &(i64, i64, Vec<i64>)) -> i64 {
    (*min..=*max).map(|target| input.iter().map(|x| (x - target).abs()).sum()).min().unwrap()
}

#[aoc(day7, part2)]
pub fn solve_part2((min, max, input): &(i64, i64, Vec<i64>)) -> i64 {
    (*min..=*max).map(|target| input.iter().map(|x| (1..=(x - target).abs()).sum::<i64>()).sum()).min().unwrap()
...
mod tests {
    ...
    fn it_works_part2() {
        ...
        // );
        // lmao this takes forever to run so i simply do not
    }
}
